...
自动化套件 679 条，连续三轮全绿，`npm run check` 通过。
...
另外还有一处**未经真机确认**：0.9.11 给所有输入框显式解开了 UXP 的
256 字符默认上限。限制本身是真的（面板上的字数计数器停在 256 就是证据），
但「UXP 认不认 `maxlength`」只在替身上验过 —— 而那个替身的 256 行为是
照着现象写的，不是从真机测出来的。
