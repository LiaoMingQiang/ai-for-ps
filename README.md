...
自动化套件 709 条，连续两轮全绿，`npm run check` 通过。
...
- **LiblibAI 云端工作流跑不通**。平台自己回
  ...
  RunningHub 那两条不受影响。
- **系统代理修复未经真机确认**。0.9.19 改成从 Windows 注册表读代理设置
  （大多数代理软件只写 WinINET、不设环境变量，而我们原来只看环境变量，
  于是那段"检测到代理就重启"从来没触发过）。注册表解析逻辑在真机上单独
  验过，但「装上之后能不能拉回模型」只有真机能确认 —— 测试不该依赖
  机器的代理设置。
