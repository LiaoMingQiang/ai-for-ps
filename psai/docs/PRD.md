...
| 云端工作流 | 下拉：13 条内置预设（按能力分类，带说明/节点数/是否需要遮罩）+「自定义工作流 ID…」。选自定义时明确提示它没有内置绑定表，需要先导入同一份图并完成绑定 |
| 模型 | 下拉（闭源时），来自「拉取模型」 |
| 状态 | ✅ 就绪 / ⚠ 缺节点 / ⚠ 缺模型 / ❌ 未配置 |
| 操作 | 恢复出厂绑定 / 依赖检查 |
...
- 并发：NORMAL 账号同时只允许跑 1 个任务，第二个提交会收到 `TASK_QUEUE_MAXED`。这**不是失败**，
  必须映射成 `PROVIDER_RATE_LIMIT` 并按 5s/15s/30s/60s/120s/120s 退避重试（累计约 6 分钟），
  期间进度条如实显示「远端队列已满，N 秒后重试」。直接判失败会让用户看到一次莫名其妙的报错，
  手动重试一下又好了 —— 这种失败最消耗信任。

#### 7.5.1 云端工作流的参数绑定（关键）

RunningHub 的云端工作流是**别人账号下的公开工作流**，我们只能通过 `nodeInfoList`
覆盖指定节点的指定字段。这带来两个必须正面处理的风险：

**风险一：空绑定等于假成功。** 不带 `nodeInfoList` 提交，云端会照跑不误，用作者预置的示例图
出一张图。接口返回成功、状态机走完、结果图也有 —— 但那张图和用户的输入毫无关系。
因此：**没有绑定表就必须拒绝提交**，并说明原因（`WORKFLOW_NOT_BOUND`）。
提交前还要确认输入图确实落进了某个节点，没落进去同样拒绝（`WORKFLOW_BINDING_INVALID`）。

**风险二：提示词可能走不到文本编码器。** 实测发现 `ArgosTranslateTextNode`（第三方中译英节点）
在 RunningHub 环境里不工作、输出空字符串。把提示词绑在它上面，任务同样「成功」，
出来的是 Flux 拿空提示词生成的文字截图。**唯一能发现这件事的办法是看图**。
因此内置预设只选提示词能顺着连线走到 `CLIPTextEncode` 的工作流，
`npm run verify:rh` 会做这条可达性检查，`npm run test:cloud:real` 则拿视觉模型当裁判，
把结果图和「应该看到什么」一起发给它判 YES/NO。

**实现方式：** 提交前先用 `POST /api/openapi/getJsonApiFormat` 拉到云端工作流的真实 API 格式图，
在这份真图上跑一遍和本地 ComfyUI 完全相同的 `applyBindings`，再与原图做差，
差异项才进 `nodeInfoList`。这样做有三个好处：节点存在性在提交前就校验了；
`sizeWidth` / `linear` / `appendText` 这些变换云端本地行为一致；
没被改动的字段不覆盖，云端作者调整默认值时用户不会被锁死在旧值上。

#### 7.5.2 内置云端预设

出厂内置 13 条经过实测校验的云端工作流预设，覆盖文生图、图生图、抠图、换背景、产品场景图、
局部重绘、扩图、高清放大、重打光、线稿上色、图片转线稿、老照片修复、万物消除。
清单与逐条节点绑定见 `docs/WORKFLOWS.md`。

每条预设都记录 `needsMask`：靠 `LoadImage` 的 MASK 输出识别处理区域的工作流（局部重绘、消除），
输入图必须带 alpha 通道，否则整张图都会被当成待处理区。提交前如实拦截并提示用户先建立选区。

`npm run verify:rh` 会拿真实 API Key 重新拉一遍每个工作流的图，逐条比对 nodeId 是否还在、
字段是否还存在、是否变成了连线输入、提示词是否还能走到文本编码器。
云端作者随时可能改图，这个校验就是用来赶在用户之前先发现的。
...
| 自动化测试 | `npm test` | 全部套件通过（含 17 个功能的页面渲染冒烟） |
| 内置工作流实测 | `npm run test:comfy:real` | 11/11 在真实 ComfyUI 上出图 |
| 云端预设核对 | `npm run verify:rh` | 13 条预设的每一条绑定都还命中云端真图 |
| 云端端到端 | `npm run test:cloud:real` | 真 Key、真工作流、真出图；含视觉裁判判定提示词是否生效 |
