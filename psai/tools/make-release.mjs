...
- **AI 应用的节点表改成逐项填写**。原来要求把整段 curl 粘进文本框，
  但真机上 UXP 的文本框**粘贴会被截断** —— 用户粘完只剩开头几行，
  换行也被吃掉，于是报「没能找到 nodeInfoList」，看起来像我们不认他的格式。
  那是宿主的行为，改不了。现在改成一行一个字段（节点号 / 字段名 / 默认值），
  一个应用通常只有两三行，不经过剪贴板就不会被截断。
  粘贴框留着当便利：能粘全就点「解析」自动填进去。
  还有第三条路「从文件读」：把那段 curl 存成 txt 再选进来，
  既不经过剪贴板、也不用一个字一个字抄。三条路填的是同一张表。

- **安装失败时那行报错不再是乱码**。一台中文用户名的机器装失败，弹出来的
  报错里用户名成了乱码，而且**用户名和 AppData 之间的反斜杠没了**。
  NSIS 用 nsExec::ExecToStack 捕获这段 stdout，按系统 ANSI 代码页解码，
  而我们写的是 UTF-8 —— 中文 Windows 上就是 GBK 乱码；更糟的是 GBK
  双字节字符的第二字节可能正好是 0x5C，被当成路径分隔符吃掉。
  现在给 NSIS 的一律压成纯 ASCII（**反斜杠一个不少**），中文细节写进日志。

- **复制插件文件失败时会先查事实再给建议**，不再原样端出 Node 的
  「EIO, Access is denied」。它会当场检查：目标在不在、父目录在不在、
  父目录能不能写，再据此说是「Photoshop 正开着占用目录」还是
  「当前账号没有写权限（AppData 被重定向到 OneDrive / 安全软件拦截）」。
  这两种的处理方式完全不同，而原来那句话两种都指不出来。
...
- **那台机器装失败的真正原因仍未查明**。这一版只是把报错从一句乱码
  变成了能照着做的话，并让它自己去查父目录可不可写 —— 究竟是
  Photoshop 开着、还是权限/重定向，要等那台机器再装一次、看新的报错才知道。
